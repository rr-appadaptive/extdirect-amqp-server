...
function Router(config) {
	...
	this.initAmqpConsumer = function(router) {
		var me = this;
		
		me.amqpConsumer = broker.RpcConsumer(config).startConsume(config, router);
	};
    ...
    this.processRoute = function(req, res){
        ...
        if(config.enableProcessors) {
            ...
            var root = __dirname.replace(vPath.normalize('/lib/extdirect-amqp-server/lib'), ''),
    ...
    this.processAmqpRequest = function(params, dxQueue, callback){
        var me = this,
        	action = dxQueue.split(':')[0],
        	method = dxQueue.split(':')[1],
        	root = __dirname.replace(vPath.normalize('/lib/extdirect-amqp-server/lib'), ''),
        	slash = platform.substr(0,3) === 'win' ? '\\' : '/', //untested
        	currentPacket = {},
        	pLen,
        	i;
        
        currentPacket.action = action;
        currentPacket.method = method;
        currentPacket.data = params;
        	
        callback2 = function() {

            return function(error, result) {
                var newResult = result !== undefined ? result : {}

                callback(null, result);
           };
        }.apply(null, [currentPacket]);
        
        currentPacket.data.push(callback2);
        
        apiPath = vPath.join(root, config.classPath, action.replace('.', slash)) + '.js';
        api = require(apiPath);
        api[method].apply({}, currentPacket.data);
    };
