...
function API(config) {
    ...
    this.createAPI = function(callback) {
        ...
        me.walk(dxPath, function(err, arr) {
            ...
            for (var dxModule in config.clientServices) {
            	response.actions[dxModule] = [];
            	for (var dxAction in config.clientServices[dxModule]) {
            		actionDescription = {
            			name: config.clientServices[dxModule][dxAction],
            			len: 1
            		}
            		response.actions[dxModule].push(actionDescription);
            	}
            	
            	if(!response.actions[dxModule].length){
                    response.actions[dxModule] = null;
                }
            }
